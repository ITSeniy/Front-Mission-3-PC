...
1. **D4 IDCT rounding + intermediate 9-bit clamp** — **landed** (Beetle
   two-pass `(sum+0x4000)>>15`, `Mask9ClampS8`, scale matrix pre-`>>3`).
2. **D5 YUV→RGB + green truncation** — **landed** (/256 coeffs, green masks,
   `^0x80`).
3. **D1 + D2 dequant <<4 domain** — **landed** (sign bias, ±0x4000 clamp).
4. **D6 4bpp packing + 8bpp xor bias** — **landed 2026-07-10 (Block C)**
   (`append_luma_block`: `min(127,y+8)` nibble pack + 0x88/0x80 xor).
5. **D9 EOB/0xFE00** — **partial (Block C)**: zig-zag CoeffIndex walk matches
   Beetle; leading FE00 skip + mid-block EOB zero-fill. Full alignment still
   needs streaming FIFO (D8).
6. **D7 status bits** — **partial (Block C)**: command bits 25–28 mirrored to
   status 23–26; `current_block = (DecodeWB+4)%6` advances per block. Remaining-
   words/busy still collapse on one-shot decode.
7. **D8 + streaming FIFO model** — still open. One-shot whole-frame buffer vs
   32-word In/Out FIFOs. Largest effort; do when a title desyncs on DMA pacing.
