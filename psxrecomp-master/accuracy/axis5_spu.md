...
### 2.6 SPU IRQ (address match) — landed (2026-07-10)
...
- **Ours:** `s_irq_address` from `0x1F801DA4`; match on decode block addr,
  DMA halfwords, FIFO halfword, capture writes; `psx_irq_raise(IRQ_SPU)`;
  sticky `s_spustat_irq9` cleared when SPUCNT.6 written 0. Edge cases (late
  IRQ on re-enable while already at address) not modeled.

### 2.7 Capture buffers — landed (2026-07-10)
...
- **Ours:** each guest mix sample writes CD pre-vol L/R + voice1/3
  `last_volume` into the four 1KB banks; `s_capture_pos` advances by 2, wraps
  at 0x400. CD ring is drained every enabled sample (for capture timing), not
  only when SPUCNT.0 is set.

### 2.8 SPUSTAT (0x1F801DAE) — partial (2026-07-10)
- **Beetle:** `SPUStatus = SPUControl & 0x3F`, plus IRQ bit (0x40), plus the
  capture-half bit and DMA/transfer-mode bits (`spu.cpp:823-827`).
- **Ours:** low-6 SPUCNT mirror + IRQ bit + always-ready `0x400` + capture-half
  `0x800` when CWA≥0x200. Full DMA request/busy mode bits still simplified
  (`spu_dma_ready` always 1).
...
5. **P1 — SPU IRQ address match (§2.6).** Landed 2026-07-10 (decode/DMA/FIFO/
   capture + SPUSTAT.6 + clear on SPUCNT.6). Optional: late-IRQ recheck on
   re-enable.
6. **P2 — Pitch modulation / PMON (§2.4).** Landed earlier (voice N−1
   `last_volume`).
7. **P2 — Capture buffers (§2.7).** Landed 2026-07-10.
8. **P2 — Real SPUSTAT (§2.8).** Partial 2026-07-10 (mirror+IRQ+half+ready);
   remaining: DMA request/busy mode bits.
