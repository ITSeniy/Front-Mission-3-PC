...
/* Guest-clock advance: PS1 SPU emits one stereo sample every 768 system
 * cycles (33.8688 MHz / 44100). Call from psx_advance_cycles device path.
 * Samples land in an internal ring; spu_render only drains that ring for
 * the host (SDL) — voice/ADSR/XA state is never stepped from host time. */
void spu_advance(uint32_t cycles);

/* Host consumer: pop guest-produced samples into out_stereo[frames*2].
 * Underrun holds the last sample (no silence clicks). Does not advance
 * guest SPU state. */
void spu_render(int16_t* out_stereo, int frames);

/* Samples waiting in the guest output ring (for host pump sizing). */
