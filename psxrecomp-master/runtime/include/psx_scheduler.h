...
/* Structured post-mortem snapshot of the scheduler and its always-on escape
 * ring. Returns the JSON byte count, or 0 when the buffer is invalid/too small.
 * `count` is clamped to the ring capacity. */
int psx_scheduler_diag_json(struct CPUState* cpu, char* out, size_t cap, int count);
