...
static int decode_rle_block(int16_t *block, const uint8_t *quant,
     ...
     * ±0x4000. CoeffIndex walks zig-zag order like Beetle WriteImageData:
     * leading 0xFE00 ignored (no block consumed — already skipped above);
     * mid-block 0xFE00 zero-fills remaining zig entries and finishes. */
    ...
    int tmp = (q != 0) ? (((ci * q) << 4) + (ci ? (ci < 0 ? 8 : -8) : 0))
                       : ((ci * 2) << 4);
    block[zigzag_to_linear[0]] = (int16_t)clamp_int(tmp, -0x4000, 0x3FFF);
    k = 1;

    while (*pos < end && k < 64u) {
        word = mdec.input[(*pos)++];
        if (word == 0xFE00u) {
            /* Mid-block EOB: remainder already zero from memset. */
            break;
        }

        uint32_t rl = (word >> 10) & 0x3Fu;
        k += rl;
        if (k >= 64u) break;
...
/* Mono EncodeImage (Beetle mdec.cpp:332-368). depth 0 = 4bpp nibble-packed,
 * depth 1 = 8bpp. Colour path never calls this. */
static void append_luma_block(const int16_t *yblk) {
    if (mdec.output_depth == 0) {
        /* 4bpp: min(127, y+8), high nibble, two pixels per byte.
         * pix_xor = unsigned ? 0x88 : 0x00 (MSB of each nibble). */
        uint8_t pix_xor = mdec.output_signed ? 0x00u : 0x88u;
        for (int i = 0; i < 64; i += 2) {
            int a = (int)yblk[i] + 8;
            int b = (int)yblk[i + 1] + 8;
            if (a > 127) a = 127;
            if (b > 127) b = 127;
            uint8_t packed = (uint8_t)(((a >> 4) & 0x0F) | (b & 0xF0));
            append_byte((uint8_t)(packed ^ pix_xor));
        }
    } else {
        /* 8bpp: y ^ (unsigned ? 0x80 : 0x00). */
        uint8_t pix_xor = mdec.output_signed ? 0x00u : 0x80u;
        for (int i = 0; i < 64; i++) {
            append_byte((uint8_t)((uint8_t)(int8_t)yblk[i] ^ pix_xor));
        }
    }
}

/* Status "current block" = (DecodeWB + 4) % 6 (Beetle mdec.cpp:518). */
static void set_decode_wb(int decode_wb) {
    mdec.current_block = (uint8_t)((decode_wb + 4) % 6);
}
...
static void execute_decode(void) {
    ...
    if (mdec.output_depth < 2) {
        ...
        while (pos < end && decode_rle_block(yblk, mdec.y_quant, &pos, end)) {
            /* Mono DecodeWB stays on Y path; report as Y0-equivalent (wb=2). */
            set_decode_wb(2);
...
uint32_t mdec_read(uint32_t addr) {
    ...
    /* Beetle MDEC_Read (mdec.cpp:755+): remaining / current-block / command
     * bits 25..28 mirrored into status 23..26 / DMA req / busy / data-out empty.
     * One-shot decode still collapses remaining+busy earlier than hardware FIFO. */
    uint32_t status = remaining_words ? ((remaining_words - 1u) & 0xFFFFu) : 0xFFFFu;
    status |= ((uint32_t)mdec.current_block & 7u) << 16;
    status |= ((mdec.command >> 25) & 0xFu) << 23;
