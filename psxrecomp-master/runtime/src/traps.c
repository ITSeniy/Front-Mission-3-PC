...
static int sched_json_append(char* out, size_t cap, size_t* pos,
                             const char* fmt, ...)
{
...
int psx_scheduler_diag_json(CPUState* cpu, char* out, size_t cap, int count)
{
    if (!cpu || !out || cap < 256u) return 0;
    if (count < 0) count = 0;
    if (count > (int)SCHED_ESCAPE_RING_CAP) count = SCHED_ESCAPE_RING_CAP;

    uint64_t total = g_sched_escape_seq;
    uint64_t avail = total < SCHED_ESCAPE_RING_CAP ? total : SCHED_ESCAPE_RING_CAP;
    if ((uint64_t)count > avail) count = (int)avail;
    uint64_t start = total - (uint64_t)count;
    size_t pos = 0;

    if (!sched_json_append(out, cap, &pos,
            "{\"current_tcb\":\"0x%08X\","
            "\"return_tcb\":\"0x%08X\","
            "\"pc0_reason\":%u,"
            "\"escape\":{\"reason\":%u,\"target_tcb\":\"0x%08X\","
            "\"resume_pc\":\"0x%08X\"},"
            "\"ring_total\":%llu,\"ring_count\":%d,\"entries\":[",
            psx_sched_current_tcb(cpu), g_sched_return_tcb, g_pc0_reason,
            (unsigned)g_sched_escape.reason, g_sched_escape.target_tcb,
            g_sched_escape.resume_pc, (unsigned long long)total, count))
        return 0;

    for (int i = 0; i < count; i++) {
        const SchedEscapeEntry* e =
            &g_sched_escape_ring[(start + (uint64_t)i) & (SCHED_ESCAPE_RING_CAP - 1u)];
        if (!sched_json_append(out, cap, &pos,
                "%s{\"seq\":%u,\"frame\":%u,\"reason\":%u,"
                "\"current_tcb\":\"0x%08X\",\"target_tcb\":\"0x%08X\","
                "\"resume_pc\":\"0x%08X\",\"pc\":\"0x%08X\","
                "\"ra\":\"0x%08X\",\"sp\":\"0x%08X\"}",
                i ? "," : "", e->seq, e->frame, e->reason,
                e->current_tcb, e->target_tcb, e->resume_pc,
                e->pc, e->ra, e->sp))
            return 0;
    }
    if (!sched_json_append(out, cap, &pos, "]}")) return 0;
    return (int)pos;
}
...
static int psx_change_thread_fiber(CPUState* cpu, uint32_t target_tcb)
{
    ...
    debug_server_log_thread_event(9, cpu, target_tcb, current_tcb, cpu->gpr[31]);
    /* The original guest context was restored above. Continue through the
     * syscall wrapper on this host stack so its CPS jr $ra publishes the
     * restored guest return address. Publishing PC=0 here discarded that
     * valid continuation and turned a completed worker thread into a
...
void psx_scheduler_run(CPUState* cpu)
{
    ...
    for (;;) {
        ...
        if (g_sched_escape.reason == PSX_RUN_RESUME_CURRENT &&
            ...
            dispatched_tcb = psx_current_tcb_ptr(cpu);
        } else {
        ...
        cpu->pc = run_pc;
        g_sched_escape.reason = PSX_RUN_CONTINUE;
        g_pc0_reason = PSX_PC0_GUEST_RETURN;
        psx_dispatch(cpu, run_pc);

        /* psx_dispatch returned with NO structured escape => the outermost
         * dispatch saw cpu->pc==0. The real BIOS can change PCB[0] while a
         * thread runs (notably from its exception/RFE scheduler) without a
         * host-side scheduler escape. In that case the zero PC completes the
         * thread we dispatched, while PCB[0] already identifies the runnable
         * context that must resume. Honor that handoff before consulting the
         * older one-level return slot. */
        uint32_t current_tcb = psx_current_tcb_ptr(cpu);
        if (psx_is_valid_tcb(cpu, current_tcb) &&
            psx_is_valid_tcb(cpu, dispatched_tcb) &&
            current_tcb != dispatched_tcb) {
            if (g_sched_return_tcb == current_tcb) {
                g_sched_return_tcb = 0; /* the outstanding handoff is consumed */
            }
            g_sched_escape.reason = PSX_RUN_YIELD_TO_TCB;
            sched_escape_ring_log(cpu, PSX_RUN_YIELD_TO_TCB,
                                  dispatched_tcb, current_tcb, 0);
            continue;
        }

        /* If a thread yielded to us and then ran to completion without moving
         * PCB[0], hand control back to its yielder (one-level safety net)
         * instead of tearing down the whole run. Otherwise this is the real
         * top-level abnormal exit. */
        if (psx_is_valid_tcb(cpu, g_sched_return_tcb) &&
            g_sched_return_tcb != current_tcb) {
            uint32_t completed = current_tcb;
        ...
        sched_escape_ring_log(cpu, PSX_RUN_GUEST_EXIT,
                              psx_current_tcb_ptr(cpu), g_sched_return_tcb, 0);
        return;
    }
}
    ...
    switch (func) {
        ...
        case 3: { /* ChangeThread / ReturnFromException */
            ...
            if (!psx_get_in_exception() && psx_change_thread(cpu, target_tcb)) {
                /* Thread switched (and this thread has since been resumed).
                 * Return 0 so the syscall wrapper falls through to its own
                 * jr $ra. The legacy fiber path has restored that guest $ra;
                 * the HLE scheduler unwinds before reaching this return. */
                return 0;
            }
