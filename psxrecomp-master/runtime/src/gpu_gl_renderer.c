...
/* 24-bit FMV present reads CPU VRAM only. Staging every A0 into the GL FBO is
 * wasted work — BUT ensure_cpu()'s full glReadPixels will wipe unstaged A0 if
 * we skip. Pair: skip GL stage while depth24, and main.cpp skips sync_cpu on
 * depth24 presents. Leaving FMV forces a full CPU→GL restage. */
static int s_fmv_cpu_only_uploads = 0;

static int gl_display_is_depth24(void) {
    GpuDisplayInfo di;
    gpu_get_display_info(&di);
    return di.depth24 != 0;
}

static void gl_resync_after_fmv_cpu_only(void) {
    if (!s_fmv_cpu_only_uploads) return;
    up_add(0, 0, VRAM_W - 1, VRAM_H - 1);
    s_fmv_cpu_only_uploads = 0;
}

static void glb_vram_write(int x,int y,uint16_t px){
    sw_vram_write(x,y,px);
    if (gl_display_is_depth24()) {
        s_fmv_cpu_only_uploads = 1;
        return;
    }
    gl_resync_after_fmv_cpu_only();
...
void gl_renderer_present_vram(int disp_x, int disp_y, int w, int h, int linear,
    ...
    /* First 15-bit present after FMV: restage CPU VRAM into the FBO. */
    gl_resync_after_fmv_cpu_only();
    flush_tex_batch();
    flush_cpu_upload();
