...
static void json_escape(const char *in, char *out, size_t outcap) {
    size_t o = 0;
    for (size_t i = 0; in && in[i] && o + 1 < outcap; i++) {
        ...
        if (ch == '\\' || ch == '"') {
            ...
            if (o + 6 >= outcap) break;
            snprintf(out + o, outcap - o, "\\u%04X", (unsigned)ch);
            o += 6;
        } else {
            out[o++] = (char)ch;
        }
...
void psx_crash_trace_dump(const char *reason, void *seh_info) {
    ...
    char reason_esc[4096];
    char exit_origin_esc[512];
    char build_esc[512];
    ...
    /* Deterministic scheduler state + escape history. This is the decisive
     * evidence for a top-level PC=0 exit and must be serialized before any
     * teardown can invalidate CPU/TCB state. */
    if (debug_cpu_ptr) {
        static char sched[131072];
        int k = psx_scheduler_diag_json(debug_cpu_ptr, sched, sizeof(sched), 256);
        if (k > 0) {
            append_str(buf, sizeof(buf), &pos, "  \"scheduler\": ");
            append_str(buf, sizeof(buf), &pos, sched);
            append_str(buf, sizeof(buf), &pos, ",\n");
        }
    }
