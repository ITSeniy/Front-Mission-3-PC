...
static void sdl_audio_pump(void) {
    ...
    if (frames < 64) return;
    if (frames > 2048) frames = 2048;
    if (queued + (uint32_t)frames * bytes_per_frame > max_bytes) {
        frames = (int)((max_bytes - queued) / bytes_per_frame);
        if (frames < 64) return;
    }
...
static void sdl_vblank_present(void) {
    ...
    /* After this present (or early-return), top up the audio queue once more.
     * FMV spikes drain the device buffer during pacer/present; a trailing pump
     * hides micro-underruns that sound like crackle. No-op if already full. */
    struct AudioTopupGuard {
        ~AudioTopupGuard() { sdl_audio_pump(); }
    } audio_topup_guard;
#endif
    ...
    bool fmv_frame = false;  /* FMV/boot — present pillarboxed 4:3 in widescreen */
    {
        ...
        /* depth24 FMV: A0 already wrote CPU VRAM. Do NOT ensure_cpu/sync —
         * full FBO readback would stomp unstaged (or just-written) movie
         * pixels. 15-bit / diagnostic CPU present still syncs. */
        if (g_gl_active && !di.depth24) gl_renderer_sync_cpu();

        /* FMV is typically ~15 fps while we vblank at 60. Re-scanning VRAM and
         * re-uploading the same frame 4× is a major stutter source. Skip the
         * expensive present when MDEC hasn't produced a new decode and the
         * display rectangle is unchanged; the previous swap stays on screen.
         * Guest time is already paced above. */
        {
            static uint32_t s_fmv_last_mdec = 0xFFFFFFFFu;
            static uint32_t s_fmv_last_dx = 0, s_fmv_last_dy = 0;
            static uint32_t s_fmv_last_w = 0, s_fmv_last_h = 0;
            static int      s_fmv_have_present = 0;
            if (!di.depth24) {
                s_fmv_have_present = 0;  /* next FMV must present its first frame */
            } else {
                uint32_t mc = mdec_get_decode_count();
                if (s_fmv_have_present
                    && mc == s_fmv_last_mdec
                    && di.display_x == s_fmv_last_dx
                    && di.display_y == s_fmv_last_dy
                    && present_w == s_fmv_last_w
                    && h == s_fmv_last_h) {
                    return;
                }
                s_fmv_last_mdec = mc;
                s_fmv_last_dx = di.display_x;
                s_fmv_last_dy = di.display_y;
                s_fmv_last_w = present_w;
                s_fmv_last_h = h;
                s_fmv_have_present = 1;
            }
        }
        ...
        if (g_vk_active) {
            if (di.depth24) {
                /* 24-bit (FMV): CPU VRAM is authoritative (A0). Skip sync_cpu
                 * for the same wipe reason as the GL path above. */
                gpu_fill_display_argb(&di, sdl_pixel_buf, present_w, present_w, h);
        ...
        if (!wide_present) {
            if (active_scale > 1) {
                ...
                /* Bulk fill (esp. 24-bit FMV) — avoids per-pixel call overhead. */
                gpu_fill_display_argb(&di, sdl_pixel_buf, present_w, present_w, h);
            }
        }
    }
...
int main(int argc, char** argv) {
  ...
  if (g_headless) {
    ...
    if (SDL_InitSubSystem(SDL_INIT_AUDIO) == 0) {
        ...
        want.channels = 2;
        /* Larger device period (~46 ms at 44.1k) absorbs brief host spikes
         * during FMV without changing guest timing. */
        want.samples = 2048;
