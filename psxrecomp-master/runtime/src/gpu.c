...
void gpu_fill_display_argb(const GpuDisplayInfo* di, uint32_t* out,
                           uint32_t stride_px, uint32_t width, uint32_t height) {
    if (!di || !out || width == 0 || height == 0) return;
    if (stride_px < width) stride_px = width;

    if (di->depth24) {
        /* 24-bit packed RGB: 3 bytes/pixel from VRAM starting at display_x*2.
         * Stream bytes from halfwords instead of 3 random lookups per pixel. */
        for (uint32_t y = 0; y < height; y++) {
            uint32_t vy = (di->display_y + y) & 511u;
            uint32_t byte0 = ((di->display_x & 1023u) * 2u);
            uint32_t* row = out + (size_t)y * stride_px;
            uint32_t nbytes = width * 3u;
            uint32_t bi = 0;
            uint32_t px = 0;
            uint8_t rgb[3];
            int rgb_i = 0;
            while (bi < nbytes) {
                uint32_t bx = byte0 + bi;
                uint16_t hw = vram[(vy * 1024u) + ((bx >> 1) & 1023u)];
                uint8_t b0 = (bx & 1u) ? (uint8_t)(hw >> 8) : (uint8_t)hw;
                rgb[rgb_i++] = b0;
                bi++;
                if (rgb_i == 3) {
                    row[px++] = 0xFF000000u
    ...
    /* 15-bit path: direct VRAM halfwords + 5→8 expansion (screen LUT if any). */
    for (uint32_t y = 0; y < height; y++) {
        uint32_t vy = (di->display_y + y) & 511u;
        uint32_t* row = out + (size_t)y * stride_px;
