...
static void handle_sched_escape_ring(int id, const char *json)
{
    ...
    char *payload = (char *)malloc(131072);
    if (!payload) { send_err(id, "oom"); return; }
    int n = psx_scheduler_diag_json(debug_cpu_ptr, payload, 131072, count);
    if (n <= 0) {
        free(payload);
        send_err(id, "scheduler snapshot unavailable");
        return;
    }
    send_fmt("{\"id\":%d,\"ok\":true,\"scheduler\":%s}", id, payload);
    free(payload);
}
